...
async function tryReportStatus(statusReport: StatusReport, retryOnAppResume?: NativeEventSubscription) {
    ...
    try {
        if (statusReport.appVersion) {
            ...
            await sdk.reportStatusDeploy(/* deployedPackage */ undefined, /* status */ undefined, previousLabelOrAppVersion, previousReleaseChannelPublicId);
        } else {
            ...
            const sdk = getPromisifiedSdk(requestFetchAdapter, config);
            await sdk.reportStatusDeploy({ ...statusReport.package, deploymentKey: statusReport.package.releaseChannelPublicId }, statusReport.status, previousLabelOrAppVersion, previousReleaseChannelPublicId);
        }
